...
try {
  ...
  await db.ref("stats").transaction((current) => {
    if (!current) {
      return {
        totalMessages: 1,
        languageDistribution: {
          [dialogData?.language || "Unknown"]: 1,
        },
        intentDistribution: {
          [intent || "Unknown"]: 1,
        },
    ...
    current.intentDistribution = current.intentDistribution || {};
    const safeIntent = intent || "Unknown";
    current.intentDistribution[safeIntent] =
      (current.intentDistribution[safeIntent] || 0) + 1;

    current.users = current.users || { whatsapp: 0, sms: 0 };
    current.users.whatsapp =
      (current.users.whatsapp || 0) + 1;

    current.last24hMessages =
      (current.last24hMessages || 0) + 1;

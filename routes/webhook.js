...
await db.ref("stats").transaction((current) => {
  if (!current) {
    return {
      totalMessages: 1,
      languageDistribution: { [detectedLang]: 1 },
      intentDistribution: { [intentName]: 1 },
      users: {
        whatsapp: 0,
        sms: 0,
      },
  ...
  current.languageDistribution[detectedLang] =
    (current.languageDistribution[detectedLang] || 0) + 1;

  current.intentDistribution = current.intentDistribution || {};
  current.intentDistribution[intentName] =
    (current.intentDistribution[intentName] || 0) + 1;

  current.last24hMessages = (current.last24hMessages || 0) + 1;
